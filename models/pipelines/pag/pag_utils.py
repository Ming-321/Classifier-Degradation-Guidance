...
class PAGMixin:
    r"""Mixin class for [Perturbed Attention Guidance](https://huggingface.co/papers/2403.17377v1)."""
    ...
    def _set_pag_attn_processor(self, pag_applied_layers, do_classifier_free_guidance):
        ...
        def is_self_attn(module: nn.Module) -> bool:
            r"""
            Check if the module is self-attention module based on its type.
            For Flux, we check if it's a FluxAttention module.
            """
        ...
        for layer_id in pag_applied_layers:
            # for each PAG layer input, we find corresponding self-attention layers in the model
            target_modules = []
    ...
    def set_pag_applied_layers(
        ...
        pag_attn_processors: Tuple[AttentionProcessor, AttentionProcessor] = None,
    ):
        r"""
        ...
        Args:
            pag_applied_layers (`str` or `List[str]`):
                ...
                PAG is to be applied. A few ways of expected usage are as follows:
                  - Single layers specified as - "blocks.{layer_index}" or "transformer_blocks.{layer_index}"
            ...
            pag_attn_processors:
                (`Tuple[AttentionProcessor, AttentionProcessor]`, optional): A tuple of two attention processors. 
                The first attention processor is for PAG with Classifier-free guidance enabled (conditional and 
                unconditional). The second attention processor is for PAG with CFG disabled (unconditional only).
                If not provided, will use default processors based on the model type.
        """
