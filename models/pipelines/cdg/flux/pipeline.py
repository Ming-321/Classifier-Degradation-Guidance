...
from diffusers.pipelines.flux.pipeline_flux import PipelineImageInput
from diffusers.pipelines.flux.pipeline_flux import calculate_shift, retrieve_timesteps
import numpy as np
from diffusers.pipelines.flux.pipeline_flux import FluxPipelineOutput
from diffusers.pipelines.flux.pipeline_flux import replace_example_docstring
from .mm_dit import CDGFluxTransformer2DModel
from ..utils import update_flux_cdg_process_params
...
class CDGFluxPipeline(FluxPipeline):
    """
    CDG enabled Flux Pipeline for text-to-image generation.

    This pipeline extends the base FluxPipeline with Context-aware Weight Generation
    capabilities that allow for selective token processing and importance-based
    ...
    Args:
        Same as FluxPipeline, with additional:
    ...
    _transformer_class = CDGFluxTransformer2DModel

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        """
        Load a pre-trained CDG Flux Pipeline from a model repository or local directory.

        This method automatically uses the CDGFluxTransformer2DModel instead of the
        ...
        Returns:
            CDGFluxPipeline: Initialized pipeline instance
        """
        # Force use of custom transformer
        transformer = CDGFluxTransformer2DModel.from_pretrained(
    ...
    def __call__(
        ...
        # 0. Parse parameters
        process_params = update_flux_cdg_process_params(process_params)
        if debug:
