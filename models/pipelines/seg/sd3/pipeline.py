...
from .mm_dit import SEGJointAttnProcessor
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import StableDiffusion3SEGPipeline
...
class StableDiffusion3SEGPipeline(StableDiffusion3Pipeline):
    """
    SEG (Self-attention Guidance) enabled Stable Diffusion 3 Pipeline.

    This pipeline extends the base StableDiffusion3Pipeline to support SEG through
    dynamic attention processor replacement. SEG perturbs image self-attention queries
    to provide additional guidance during generation.
    ...
    Args:
        Same as StableDiffusion3Pipeline, with additional SEG parameters available
        in the __call__ method (seg_scale, seg_blur_sigma, seg_applied_layers_index)
    """
    ...
    def __call__(
        ...
        Args:
            ...
            seg_scale (`float`, *optional*, defaults to 3.0):
                The scale factor for SEG (Self-attention Guidance). Set to 0 to disable SEG. Higher values increase
                the guidance strength. Typical range: 0.5-3.0.
            seg_blur_sigma (`float`, *optional*, defaults to 9999999.0):
                Sigma value for Gaussian blur applied to image query in SEG. Values greater than 9999.0 result in
                uniform blur (infinite sigma). For visible Gaussian blur effects, use smaller values like 1.0-3.0.
            seg_applied_layers_index (`List[int]`, *optional*):
                ...
                (first transformer block). SD3 has 24 transformer blocks (indices 0-23). Example: [0, 1, 2] applies
                SEG to the first three blocks.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_seg:
                    replace_processor = SEGJointAttnProcessor(
                ...
                if self.do_classifier_free_guidance and not self.do_seg:
                    ...
                    noise_pred = noise_pred_text + (self.guidance_scale - 1.0) * (noise_pred_text - noise_pred_uncond) + self.seg_scale * (noise_pred_text - noise_pred_text_perturb)
                    
                    # Skip layer guidance still supported in CFG+SEG mode if needed
                    should_skip_layers = (
                        True
                        if i > num_inference_steps * skip_layer_guidance_start
                        and i < num_inference_steps * skip_layer_guidance_stop
                    ...
                    if skip_guidance_layers is not None and should_skip_layers:
                        ...
                        noise_pred_skip_layers = self.transformer(
                            hidden_states=latent_model_input_single,
